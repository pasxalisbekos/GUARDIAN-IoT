...
def plot_cv_results(cv_results_df):
    ...
    plt.tight_layout()
    plt.savefig('cross_validation_results_dos.png')
    plt.show()
...
def compare_models(models, X_train, X_test, y_train, y_test, dataset_name):
    ...
    with open(f"./{dataset_name}_confusion_matrices_dos_source.json", 'w') as f:
...
def evaluate_models_on_target(saved_models, X_test_target, y_test_target, dataset_name):
    ...
    with open(f"./{dataset_name}_confusion_matrices_target_dos.json", 'w') as f:
...
def fetch_src_dataset(folder_malicious='/home/cpitumpeappu/wireless/CSV_DATASETS/CIC_IOT_Dataset2023/CSV_FILES/Dos',
...
source_ciciot_dos_dataset = fetch_src_dataset()
y = source_ciciot_dos_dataset["Label"]
X = source_ciciot_dos_dataset.drop(['Label'], axis=1)

del source_ciciot_dos_dataset
gc.collect()
...
cv_results_df.to_csv('./cross_validation_results_dos.csv', index=False)
...
results_df = compare_models(models, X_train_source, X_test_source, y_train_source, y_test_source, "DOS_CICIOT")
print(results_df)
results_df.to_csv('./model_performance_on_source_dos.csv')
...
def get_iot_network_intrusion_datasets():
  ...
  iot_network_intrusion_dos['Label'] = 1
  ...
  iot_network_intrusion_dos = iot_network_intrusion_dos.sample(n=int(iot_network_intrusion_normal.shape[0]), random_state=42)

  dos_w_benign = pd.concat([iot_network_intrusion_dos,iot_network_intrusion_normal])
  dos_w_benign = dos_w_benign.sample(frac=1, random_state=42).reset_index(drop=True)

  return dos_w_benign


dos_w_benign = get_iot_network_intrusion_datasets()
y = dos_w_benign["Label"]
X = dos_w_benign.drop(['Label'], axis=1)
...
target_results.to_csv('./model_performance_on_target_IoTNetIntrusion_DOS.csv')

def fetch_mqt(folder_malicious='/home/cpitumpeappu/wireless/CSV_DATASETS/DoS-DDoS-MQTT-IoT_Dataset/CSV_FILES/Dos',
...
print(target_results)
target_results.to_csv('./model_performance_on_target_MQTT_DOS.csv')




overall = pd.concat([mqtt, dos_w_benign])
y = overall["Label"]
...
target_results = evaluate_models_on_target(saved_models, X_test_target, y_test_target,"MQTT")
print(target_results)
target_results.to_csv('./model_performance_on_target_all_DOS.csv')
